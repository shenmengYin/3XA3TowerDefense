...
public class Artist_Swing extends Helper{
		...
		for(int i=0; i<mapWidth; i++)
		{
			for(int j=0; j<mapHeight; j++)
			{
				...
				if(tileType==TDMap.PATH){
					drawFilledQuad(g,new Color(127, 203, 242), i*scaledWidth, j*scaledHeight, scaledWidth, scaledHeight);
					drawEmptyQuad(g, Color.gray, i*scaledWidth, j*scaledHeight, scaledWidth, scaledHeight);
					g.drawImage(tdMap.getPathImg(),i*scaledWidth, j*scaledHeight, scaledWidth, scaledHeight, null );
				//if we have a scenery tile, draw it green.
				}else{
					drawFilledQuad(g,new Color(0, 47, 165), i*scaledWidth, j*scaledHeight, scaledWidth, scaledHeight);
					drawEmptyQuad(g, Color.GRAY, i*scaledWidth, j*scaledHeight, scaledWidth, scaledHeight);
					g.drawImage(tdMap.getWallImg(),i*scaledWidth, j*scaledHeight, scaledWidth, scaledHeight, null );
				}
			}
		}
	}
     ...
     */
    	public static void drawCritter(Critter crit, Graphics g){
			...
			//replace critter with image

			g.drawImage(crit.getImage(), posX, posY, size + 2*bufferSize, size + healthBarGap + 2*bufferSize, null);


			//if the critter is burning, we draw an Orange around it
			if(crit.isBurning()){
				//drawEmptyQuad(g, Color.orange, posX, posY, size, size);
				g.drawImage(crit.getFireEff(), posX, posY, size + 2*bufferSize, size + healthBarGap + 2*bufferSize, null);
			}
	...
	public static void drawTower(Tower tow, Graphics g){
		...
		//replace tower with image
		int n = 0;
		g.drawImage(tow.getImage(n), tow.getPosX(), tow.getPosY(), tileWidth, tileHeight, null);
    ...
    public static void drawShot(Tower tow, Critter crit, Graphics g){
		...
		//we set the stroke to be thicker than usually the level of tower
		Graphics2D g2d = (Graphics2D) g;
		g2d.setStroke(new BasicStroke(2*tow.getLevel()));
