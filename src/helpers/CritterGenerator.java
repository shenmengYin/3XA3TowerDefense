...
import models.Critter_Hunter;
import models.Critter_Mage;
import models.Critter_Zombie;
import models.Critter_Dragon;
import models.Critter_Knight;
import models.Critter_Gargoyle;
import models.TDMap;
...
public class CritterGenerator extends Helper {
	...
	private final static int MAXWAVE = 10; //THE maximum wave number
	//constructor
     ...
     */
		...
		if(waveLevel % 5 ==0){
			for(int i = 0; i < 0.5*amountOfCritters; i ++){
				critterGroup.add(new Critter_Zombie(waveLevel, exampleMap));
				critterGroup.add(new Critter_Dragon(waveLevel, exampleMap));
			}
		}
		 // We generate fast (Arrow) critters	
		 else if(waveLevel % 5 == 4){
			 for(int i = 0; i < amountOfCritters; i ++){
			critterGroup.add(new Critter_Hunter(waveLevel, exampleMap));
			 }
		...
		// We generate Resistive (X) critters	
			...
			for(int i = 0; i < (amountOfCritters); i ++){
				critterGroup.add(new Critter_Gargoyle(waveLevel, exampleMap));
			...
			for(int i = 0; i < amountOfCritters; i ++){
			critterGroup.add(new Critter_Knight(waveLevel, exampleMap));
			}
		...
		// We generate average (circle) critters	
			...
			for(int i = 0; i < amountOfCritters; i ++){
				critterGroup.add(new Critter_Mage(waveLevel, exampleMap));
